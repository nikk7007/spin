...
(function () {
  ...
  function renderTweaks() {
    window.SpinTweaks.build(tweaks, (key, value) => {
      ...
      renderTweaks(); // re-render to reflect active states
    });
  }
  ...
  function applyTweaks() {
    ...
    if (histPanel) histPanel.hidden = !tweaks.showHistory || history.length === 0;
  ...
  function renderList() {
    ...
    items.forEach(it => {
      ...
      sw.title = 'trocar cor';
      sw.addEventListener('click', () => cycleColor(it.id));
  ...
  function showResult(winner) {
    ...
    document.getElementById('result-meta').textContent = `${items.length} concorrentes · ${new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}`;
    ...
    // history
    history.unshift({ label: winner.label, color: winner.color, at: Date.now() });
